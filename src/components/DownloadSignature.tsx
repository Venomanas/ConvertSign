      ...
      {signatureFiles.length > 0 ? (
        <ul className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {signatureFiles.map(file => (
            <li key={file.id} className="bg-white p-4 rounded-lg shadow-md">
              <p className="text-gray-800 font-semibold">{file.name}</p>
              <button
                onClick={() => handleDownload(file)}
                className="mt-2 w-full py-2 px-4 bg-indigo-600 hover:bg-indigo-700 dark:bg-slate-600 dark:hover:bg-slate-700 text-white rounded-md transition-colors"
              >
                Download Signature
              </button>
            </li>
